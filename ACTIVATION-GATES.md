...
**Current evidence.** Local checks pass with 18 test files and 80 sanitized tests. Tests cover signature freshness/tampering, directory normalization and inactive/bot/mismatch rejection, and staff/channel acknowledgment boundaries. The admin mapping preview requires every row to be checked, re-resolves the directory, rejects duplicate Slack IDs, updates mappings atomically, and writes a count-only audit event. Provider configuration, hosted webhook verification, real mapping review, and the one staff-only canary remain pending. Gate status confidence: **60%**.
