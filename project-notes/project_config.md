...
- Implementation: the web runtime accepts the bot token, signing secret, staff member IDs, and staff channel ID through file-first secret settings. The one-shot `pnpm slack:validate-uat` command performs only authenticated read checks by default. Its explicit `--send-staff-canary` mode reserves an owner-only receipt before sending one fixed, learner-free message and refuses another send while that receipt exists.
- Verification: 18 test files and 80 sanitized tests passed with zero Svelte/TypeScript diagnostics. New coverage proves directory normalization and inactive/bot/mismatched-account rejection plus staff/channel/thread acknowledgment boundaries. No Slack app, hosted secret, event subscription, mapping, or real message was created during this verification.
