...
- Verification: 27 test files and 103 sanitized tests passed with zero Svelte/TypeScript diagnostics. Regressions prove the read-only preview does not call the Sheet write port or a database transaction and prove incomplete, duplicate, externally effectful, or unresolved evidence fails closed. Web and worker production builds and Compose rendering passed. UAT commit `40ed8cb` passed CI run `29788315542` and Vercel deployment run `29788315619`; the stable protected alias returned `status=ok` with integrations inactive and the automation workspace returned its expected authentication redirect. The DigitalOcean worker was rebuilt at the same commit and reached healthy with a read-only filesystem, zero restarts, a nonempty mode-`0600` heartbeat, `PHASE3_EXTERNAL_EFFECTS=false`, and no local database.
...
- Constraints: the Gate 8 artifact is deployed only to UAT with external effects disabled. This deployment authorizes no cohort mode change, provider call, Sheet write, production promotion, or Gate 8 clearance. The July 24 rehearsal and five formal September dry-run days remain timed evidence.
