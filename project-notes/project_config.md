...
- Verification: Prisma formatting, generation, and schema validation passed; Svelte/TypeScript reported zero diagnostics; 25 test files and 100 sanitized tests passed. The authorized dev/UAT migration applied successfully, and an independent status check found all four committed migrations current. PR #13 merged the artifact into uat. The protected Vercel workflow deployed the exact merge commit, reassigned the stable alias, and returned healthy with integrations inactive; both new learner routes returned authenticated redirects. The branch-scoped non-sensitive preference URL is present only for Preview uat. The DigitalOcean worker was rebuilt at the same commit and verified healthy with a nonempty heartbeat, zero restarts, no local database, and external effects false. No real message, Sheet write, or learner contact occurred.
...
- Constraints: the migration and artifact are deployed only to dev/UAT with external effects disabled. Before Gate 10, publish the notice and prove the deployed active send path rechecks preferences immediately before provider contact. Production migration remains separately authorized.

## Gate 8 dry-run evidence hardening

- Date: 2026-07-20
- Scope: UAT worker reconciliation previews, dry-run day certification, and staff-review evidence.
- Decision: a complete dry-run must perform bounded authoritative Sheet reads through the Viewer-only adapter while making zero Sheet writes and zero provider calls. Staff-entered counts supplement but cannot replace system evidence.
- Implementation: dry-run reconciliation jobs now plan the same Sheet-to-Postgres changes as active reconciliation without applying them. Day certification requires the exact nine daily jobs, completed dry-run payloads, exactly one zero-effect audit per job, no duplicate jobs, no unresolved Sheet mappings, and zero staff-observed issues. Certification emits a count-only staff-review audit.
- Verification: 27 test files and 103 sanitized tests passed with zero Svelte/TypeScript diagnostics. Regressions prove the read-only preview does not call the Sheet write port or a database transaction and prove incomplete, duplicate, externally effectful, or unresolved evidence fails closed. Web and worker production builds and Compose rendering passed. UAT implementation commit `40ed8cb` passed CI run `29788315542` and Vercel deployment run `29788315619`; evidence PR #15 produced merge commit `15e7351`, which passed final CI and Vercel deployment runs `29788885630` and `29788885684`. The stable protected alias returned `status=ok` with integrations inactive and the automation workspace returned its expected authentication redirect. The DigitalOcean worker was rebuilt at `15e7351` and reached healthy with a read-only filesystem, zero restarts, a nonempty mode-`0600` heartbeat, `PHASE3_EXTERNAL_EFFECTS=false`, and no local database. Count-only preflight initially found that its mounted database file still targeted the sanitized one-learner development database. The verified owner-only Neon UAT file showed the expected disabled cohort with 14 learners and 14 Slack mappings; it replaced the stale file atomically after a mode-`0600` backup, and the recreated worker returned the same verified counts.
- Recovery: change the cohort to `DISABLED`, stop the worker if it continues claiming jobs, inspect bounded status/error counts, fix on a branch from `uat`, and repeat a full day. Never convert a partial day into completed evidence.
- Constraints: the Gate 8 artifact is deployed only to UAT with external effects disabled. This deployment authorizes no cohort mode change, provider call, Sheet write, production promotion, or Gate 8 clearance. Count-only preflight found no July 24 program session and no approved `late`, `no_call_no_show`, or `exit_reminder` template; import/confirmation and template approval are required before dry-run mode is enabled. The July 24 rehearsal and five formal September dry-run days remain timed evidence.
