...
- Constraints: the migration is applied only to dev/UAT, and the artifact is intentionally undeployed. Before Gate 10, publish the notice, configure the environment-specific stable URL, and prove the deployed active send path rechecks preferences immediately before provider contact. Production migration remains separately authorized.
