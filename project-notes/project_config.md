...
- Verification: local contract tests cover domain/account authorization, token hashing, one-time token comparison, PKCE output, eight-hour expiry, and seven UAT provisioning guards. The approved UAT apply created one account and role without logging identifying values; a second apply returned preserve/preserve. OAuth reached account authorization before provisioning, and a post-provisioning organization sign-in retry remains pending.
